...
const SHOP_LOCATION = 'Comfort 5, First Floor, Shop F8, Nairobi, Kenya';
...
export default function CheckoutPage() {
  ...
  const selectedPayment = PAYMENT_OPTIONS.find((option) => option.id === paymentId) ?? PAYMENT_OPTIONS[1];
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4 py-12">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 sm:p-10 max-w-lg w-full text-center">
          ...
          <h2 className="text-2xl font-black text-navy mb-1">Order Received!</h2>
          <p className="text-brand font-bold mb-4">Order #{orderNumber}</p>
          <p className="text-gray-500 text-sm mb-4">
            Thank you, <strong>{details.name}</strong>. Someone from Liastute will reach out to you on{' '}
            <strong>{details.phone}</strong> to discuss delivery details, how your parcel will arrive, and anything else you need to know.
          </p>
          <div className="bg-blue-50 rounded-xl p-4 mb-4 text-sm text-gray-600 text-left">
            <p className="font-bold text-navy mb-1">Prefer to visit the shop?</p>
              ...
              <div className="space-y-1 text-gray-600">
                {placedOrderItems.map((item) => (
                  <div key={item.id} className="flex justify-between gap-3">
                    ...
                    <strong className="text-navy shrink-0">KSh {(item.price * item.quantity).toFixed(2)}</strong>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-6 text-sm text-gray-600">
            Order total: <strong className="text-navy">KSh {(placedOrderTotal ?? grandTotal).toFixed(2)}</strong>
          </div>
  ...
  const inputClass = 'w-full border border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-700 placeholder-gray-400 outline-none focus:border-brand focus:ring-1 focus:ring-brand';
  const labelClass = 'block text-sm font-bold text-navy mb-2';

  return (
    <form onSubmit={handlePlaceOrder} className="max-w-7xl mx-auto px-4 py-10 pb-16">
      <div className="grid lg:grid-cols-[1fr_440px] gap-8 lg:gap-10 items-start">
        <section className="border-t border-gray-200 pt-8">
          <h1 className="text-xl font-black text-navy mb-5 uppercase">Checkout Details</h1>
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className={labelClass}>Special notes for delivery and Exact location (optional)</label>
              <textarea
                name="notes"
                value={details.notes}
                onChange={handleDetailsChange}
        ...
        <aside className="border-2 border-navy bg-white p-6 sm:p-8 lg:sticky lg:top-32">
          <h2 className="text-xl font-black text-navy mb-5 uppercase">Your Order</h2>
          ...
          <div className="divide-y divide-gray-100">
            {items.map((item) => (
              <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4 py-4 text-sm">
                <div className="font-semibold leading-snug text-gray-600">
                  {item.name} <span className="text-navy">x {item.quantity}</span>
                </div>
            ...
            <div>
              ...
              <div className="space-y-3 rounded-xl bg-gray-50 p-4">
                {SHIPMENT_NOTES.map((note) => (
                  <div key={note.label} className="text-sm font-semibold text-gray-700">
                    <p>
                      {note.label}
                      {note.fee > 0 ? <span>: KSh {formatMoney(note.fee)}</span> : <span> - FREE</span>}
                    </p>
                    <p className="text-xs font-semibold text-gray-500">{note.description}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="border-t border-gray-100 pt-4 space-y-4">
            {PAYMENT_OPTIONS.map((option) => (
              <label key={option.id} className="flex cursor-pointer items-center gap-3 text-sm font-bold text-navy">
                <input
                  type="radio"
                  name="payment"
                  value={option.id}
